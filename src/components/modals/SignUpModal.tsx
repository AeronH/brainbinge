...
export function SignUpModal({ open, onOpenChange, initialMode = "signup" }: SignUpModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error.message || `Failed to ${isSignIn ? "sign in" : "sign up"}`);
    } finally {
      setLoading(false);
    }
  };
          ...
          {!isSignIn && (
            <div className="space-y-2">
              ...
              <Input
                ...
                className="bg-muted border-border rounded-xl h-11 text-base px-4"
                required
              />
            </div>
          )}
          ...
          >
            {isSignIn ? (
              <>
                ...
                <span className="text-blue hover:underline font-medium">Sign up</span>
              </>
            ) : (
              <>
                Already have an account?{" "}
                <span className="text-blue hover:underline font-medium">Sign in</span>
              </>
            )}
          </button>
        </div>
